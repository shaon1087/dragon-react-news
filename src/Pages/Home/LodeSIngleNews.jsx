...
const LodeSingleNews = ({ singleNews }) => {
  const { title, image_url, author, details, _id, total_view, rating } =
    singleNews;
  const { name, published_date, img } = author;
  const { number, badge } = rating;
  return (
    <div className="rounded-md my-5 space-y-3 border p-2">
      <div className="author bg-[#F3F3F3] flex gap-2 justify-between items-center p-2 rounded-md">
        <div className="flex gap-2 items-center justify-between">
          ...
          />
          <div>
            <h2>{name ? name : "Not Defined"}</h2>
            <p>{published_date ? published_date : "Not Defined"}</p>
          </div>
        </div>
      ...
      {details.length > 200 ? (
        <p>
          ...
          <Link
            to={`/news/${_id}`}
            value={_id}
            className="text-lg font-semibold text-blue-500 hover:text-red-500"
          >
            Read More
          </Link>
        </p>
      ...
      <div className="flex justify-between items-center border-t border-b p-2">
        <div className="ratting">
          <p>
            ...
            <sup className="text-orange-500 p-1">
              {badge ? badge : "Not Yet"}
            </sup>
          </p>
        </div>
